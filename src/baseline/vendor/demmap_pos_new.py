...
# Set to 1 to disable the inner process pool and let Dask own parallelism.
# Set to N>1 to use an inner pool with at most N worker processes.
HPCP_INNER_PROCS = max(1, int(os.environ.get("HPCP_INNER_PROCS", "1")))

# Number of DEM rows handled per inner task (when pooling is enabled).
HPCP_INNER_CHUNK = max(1, int(os.environ.get("HPCP_INNER_CHUNK", "100")))
...
def demmap_pos(dd, ed, rmatrix, logt, dlogt, glc,
    ...
    # ---- inner parallelism policy ----
    # If HPCP_INNER_PROCS == 1 → no inner pool (let Dask own parallelism).
    # If HPCP_INNER_PROCS > 1 and na is large enough → use a bounded inner pool.
    use_inner_pool = (HPCP_INNER_PROCS > 1) and (na >= 2 * HPCP_INNER_CHUNK)

    if use_inner_pool:
        n_par = HPCP_INNER_CHUNK
        niter = int(np.floor(na / n_par))

        # Ensure NumPy/BLAS threads don't oversubscribe inner processes
        with threadpool_limits(limits=1):
            with ProcessPoolExecutor(max_workers=HPCP_INNER_PROCS) as exe:
                futures = [
                    exe.submit(
                        dem_unwrap,
                ...
                # progress over completion (order is restored below)
                kwargs = {
                    "total": len(futures),
                    "unit": " x10^2 DEM",
                    "unit_scale": True,
        ...
        for i, f in enumerate(futures):
            r0, r1, r2, r3, r4 = f.result()
            dem[i * n_par: (i + 1) * n_par, :] = r0
            edem[i * n_par: (i + 1) * n_par, :] = r1
            elogt[i * n_par: (i + 1) * n_par, :] = r2
            chisq[i * n_par: (i + 1) * n_par] = r3
            dn_reg[i * n_par: (i + 1) * n_par, :] = r4

        # Remainder serially (small tail)
        if (na % (niter * n_par)) != 0:
            i_start = niter * n_par
        ...
        # serial path (or let outer framework parallelize tiles)
        for i in range(na):
            result = dem_pix(
                dd[i, :],
                ed[i, :],
...
def dem_unwrap(dn, ed, rmatrix, logt, dlogt, glc,
               reg_tweak=1.0, max_iter=10, rgt_fact=1.5, dem_norm0=0,
               nmu=42, warn=False, l_emd=False, rscl=False):
    # this function serialises the parallel blocks
    ndem = dn.shape[0]
    nt = logt.shape[0]
    nf = dn.shape[1]
...
def dem_pix(dnin, ednin, rmatrix, logt, dlogt, glc,
            reg_tweak=1.0, max_iter=10, rgt_fact=1.5, dem_norm0=0,
            nmu=42, warn=True, l_emd=False, rscl=False):
    nf = rmatrix.shape[1]
    nt = logt.shape[0]
    # nmu=42
