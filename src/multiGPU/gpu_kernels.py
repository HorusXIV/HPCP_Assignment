...
# Kernel fusion for frequently used operations
@cp.fuse()
def fused_filter_coefficients(s_b, lambs_vec):
    """Fused computation of filter coefficients for better performance."""
    beta_b = 1.0 / cp.sqrt(1.0 + s_b ** 2)
    alpha_b = s_b * beta_b
    denom = (alpha_b ** 2 + beta_b ** 2 * lambs_vec)
    # Add numerical stability
    denom = cp.maximum(denom, cp.finfo(cp.float64).eps)
    return alpha_b / denom, alpha_b, beta_b


@cp.fuse()
def fused_residuals_chisq(dn_b, dn_reg_device, ed_b, nf):
    """Fused computation of residuals and chi-square.

    Parameters
    - dn_b: batch of observed data (batch, nf)
    - dn_reg_device: batch of reconstructed data (batch, nf)
    - ed_b: batch of errors (batch, nf)
    - nf: number of features (scalar) to avoid using .shape inside fuse
    """
...
@cp.fuse()
def fused_svd_coeffs(s_b):
    """Fused computation of SVD-derived coefficients."""
    beta_b = 1.0 / cp.sqrt(1.0 + s_b ** 2)
    alpha_b = s_b * beta_b
    return alpha_b, beta_b
...
def demmap_pos(
    ...
    if GPU_AVAILABLE:
        try:
            ...
            have_memcpy_async = (
                hasattr(cp, "cuda") and hasattr(cp.cuda, "runtime")
            )
            have_pinned = (
            ...
            if streams_enabled:
                ...
                for _k in range(buf_depth):
                    ...
                    pinned_pool.append({
                        'dem': host_dem, 'dem_mem': mem_dem,
                        'dnreg': host_dnreg, 'dnreg_mem': mem_dnreg,
                        'chi': host_chi, 'chi_mem': mem_chi,
                    })
            ...
            while b0 < na:
                ...
                try:
                    ...
                    # Use workspace manager for efficient matrix allocation
                    C = _gpu_workspace.get_workspace_with_pool(
                        (cur_batch, M, M), cp.float64, "svd_matrix"
                    )
                    if C is None or C.shape != (cur_batch, M, M):
                        # Fallback if workspace allocation fails
                    ...
                    _range_push("SVD")
                    # Use economic SVD for better performance (20-40% speedup)
                    if streams_enabled:
                        ...
                        u_b, s_b, vh_b = cp.linalg.svd(C, full_matrices=False)
                    # Use fused kernel for coefficient computation
                    if streams_enabled:
                        with compute_stream:  # type: ignore[union-attr]
                            alpha_b, beta_b = fused_svd_coeffs(s_b)
                    else:
                        alpha_b, beta_b = fused_svd_coeffs(s_b)
                    _range_pop()
                    ...
                    if disable_vector:
                        ...
                        if streams_enabled:
                            with compute_stream:  # type: ignore[union-attr]
                                ...
                                filt, alpha_b_recomputed, beta_b_recomputed = (
                                    fused_filter_coefficients(s_b, lambs_vec)
                                )
                            ...
                            U_filt = u_b * filt[:, None, :]
                        _range_pop()
                        
                        _range_push("DATA_EXTENSION")
                        # Use workspace for data extension to reduce overhead
                        dn_ext = _gpu_workspace.get_workspace_with_pool(
                            (cur_batch, M), dn_b.dtype, "data_extension"
                        )
                        if dn_ext is None or dn_ext.shape != (cur_batch, M):
                            # Fallback allocation
                        ...
                        if streams_enabled:
                            with compute_stream:  # type: ignore[union-attr]
                                ...
                                # dem_out = (kdag^T) @ dn_ext
                                # Use optimized einsum for large arrays
                                dem_out_device = cp.einsum(
                                    'bmn,bm->bn', kdag, dn_ext
                                )
                        else:
                            kdag = cp.matmul(U_filt, W_stack)
                            dem_out_device = cp.einsum(
                        ...
                        if streams_enabled:
                            with compute_stream:  # type: ignore[union-attr]
                                ...
                                # Use fused kernel for residuals & chisq
                                # computation
                                try:
                                    residuals_device, chisq_device = (
                                        fused_residuals_chisq(
                                            dn_b, dn_reg_device, ed_b, nf_dev
                                        )
                                    )
                                except Exception:
                                    # Fallback to standard CuPy ops if fusion
                                    # fails
                                    residuals_device = (
                                        (dn_b - dn_reg_device) / ed_b
                                    )
                                    chisq_device = (
                                        cp.sum(
                                            residuals_device ** 2, axis=1
                                        ) / nf_dev
                                    )
                        else:
                            dn_reg_device = cp.matmul(
                        ...
                        if not streams_enabled:
                            ...
                            dem_batch_host = cp.asnumpy(
                                dem_out_device[:, :nt_dev]
                            )
