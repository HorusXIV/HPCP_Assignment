...
### 6. CUDA Streams and Async Transfers (new)

We added optional CUDA streams with double-buffered asynchronous D2H copies
to overlap kernel execution with host transfers in `gpu_kernels.demmap_pos`.

- Controls:
	- `MULTIGPU_STREAMS=1` enables the stream pipeline (default 1)
	- `MULTIGPU_STREAMS_DEPTH=2` sets the number of pinned host staging buffers
		to round-robin (2 or 3 recommended)

- Behavior:
	- A compute stream runs SVD, lambda selection, and reconstruction.
	- A copy stream performs device-to-host memcpyAsync into pinned buffers.
	- While copy of batch N runs on the copy stream, the compute stream starts
		batch N+1. Final CPU copies from pinned buffers to pageable outputs are
		overlapped with GPU compute.

- Fallbacks:
	- If CuPy lacks `Stream` or `alloc_pinned_memory` (e.g., test shims), the
		code automatically falls back to the original synchronous path.

- Profiling:
	- Use Nsight Systems with `NSYS_OPTS` including `cuda,nvtx` and set
		`MULTIGPU_NVTX=1`. You should see the `compute_stream` and `copy_stream`
		running concurrently with minimal idle gaps.

Future work: add H2D prefetching with a device staging buffer to overlap
host-to-device transfers for the next batch, and consider CUDA-aware MPI to
avoid CPU staging on collectives.
